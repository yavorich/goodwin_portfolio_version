...
from apps.finance.models import OperationMessage
from apps.finance.models.operation_type import (
    MessageType,
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for template_message_type in MessageType.values:
            ...
            template_message = OperationMessage.objects.filter(
                message_type=template_message_type
            ).first()
            if template_message is None:
                template_message = OperationMessage.objects.create(
                    message_type=template_message_type,
        ...
        OperationMessage.objects.exclude(pk__in=template_message_list).delete()
        print("Success")
