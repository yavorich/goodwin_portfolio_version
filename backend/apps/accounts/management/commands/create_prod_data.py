...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        with connect(settings.BASE_DIR / "goodwinprod.sqlite3") as connection:
            ...
            # create_user_program_accruals(cursor)
            # create_operation_history_withdraw(cursor)
            # create_operation_history_partner(cursor)
            # create_operation_history_program_replenishment(cursor)
            # create_operation_history_replenishment(cursor)
            # create_operation_history_extra_fee(cursor)

            create_operation_history_start_close_program(cursor)
            # imitation_working_app(cursor)
    ...
    for user in User.objects.all():
        ...
        for program_id in (
            ...
            for user_program in c_ups:
                ...
                user_program.profit -= deposit
                # user_program.save()




# def create_operation_history_start_program():
#     OperationHistory.objects.create(
#         wallet=self.wallet,
#         type=OperationHistory.Type.TRANSFER_BETWEEN,
#         description=dict(
...
# def create_operation_history_close_program():
#     if self.partial:
#         description = dict(
...
def imitation_working_app(cursor):
    for user in User.objects.all():
        ...
        user_program_extra_operations = {
            (
                get_datetime_from_iso(created_at).date() + timezone.timedelta(days=3),
                User.objects.get(email=email).pk,
                Program.objects.get(name=program_name).pk,
            ): Decimal(deposit)
            for email, deposit, created_at, program_name in cursor.fetchall()
        }

        cursor.execute(
            "SELECT email, amount_gc AS amount, finished "
            "FROM invoice "
            "JOIN user ON invoice.user_id = user.id "
            "WHERE invoice.status = '2'"
        )
        ...
        for i in range(total_days + 1):
            ...
            for program_id in programs:
                ...
                today_created_ups = list(
                    filter(lambda up: up.created_at.date() == current_date, p_cur_ups)
                )

                if len(today_created_ups) > 0:
                    print(len(today_created_ups))

                # if (
                #     current_date,
                #     user.pk,
                #     program_id,
                # ) in user_program_extra_operations.keys():
                #     amount = user_program_extra_operations[
                #         (current_date, user.pk, program_id)
                #     ]
                #
                #     if len(p_cur_ups) > 1:
                #         pass
                #         # print(p_cur_ups, amount)
                #
                #     else:
                #         user_program = p_cur_ups[0]
                #         if current_date == user_program.created_at
                #
                #         user_program_data = user_programs_data[user_program.pk]
                #         user_program_data[1] += amount

                # for user_program in p_cur_ups:
                #     user_program_data = user_programs_data[user_program.pk]
                #
                #     if current_date in user_program_data[2].keys():
                #         user_program_data[1] += user_program_data[2][current_date][0]
                #         # print(
                #         #     user_program_data[1], user_program_data[2][current_date][1]
                #         # )
                #
                #     if current_date in user_program_data[3].keys():
                #         user_program_data[1] += user_program_data[3][current_date]

        # print(
        #     [
        #         (upd[1], get_upe_deposit(cursor, upd[0]))
...
def get_upe_deposit(cursor, user_program):
    ...
    return next(
        map(
            lambda x: x[0],
            filter(
                lambda X: get_date_time_from_iso_or_null(X[1])
